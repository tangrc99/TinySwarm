...
void RPCInterface::fork(::google::protobuf::RpcController *controller, const ::ForkInput *request, ::ForkEcho *response,
    ...
    if (!owner->isServiceExist(request->service())) {
        ...
        sd->set_type(request->type());
        response->set_fail(true);
        response->set_error_text("Service Not Exist");
        return;
    }
    ...
    auto pid = owner->createProcess(request->service(), request->alias(), request->owner(),
                                    request->type() == host ? 0 : 1, exe_params, docker_params,
                                    error_text, request->restart());
    ...
    if (pid > 0) {
        ...
        // 存储进程的启动信息
        owner->addServiceRecord(request);
...
void
RPCInterface::shutdown(::google::protobuf::RpcController *controller, const ::ShutdownInput *request, ::Echo *response,
                       ::google::protobuf::Closure *done) {
    ...
    if (request->sd().type() == ServiceType::host) {
        res = owner->stopService(request->sd().alias(), request->user_info());
    } else {
        res = owner->stopDockerService(request->sd().alias(), request->user_info());
    }
    ...
    switch (res) {
        case -2:
            response->set_error_text("alias not found");
            break;
        case -1:
            response->set_error_text("kill error");
            break;
        case 0:
            response->set_error_text("success");
            break;
        case 1:
            response->set_error_text("wrong user info");
            break;
        default:
            break;
    }
...
void RPCInterface::check(::google::protobuf::RpcController *controller, const ::CheckInput *request,
    ...
    std::list<ServiceExitInformation> exits;
